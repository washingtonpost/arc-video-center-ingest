...
const IMPORT_DOMAIN_FORMAT_PROD = 'api.%s.arcpublishing.com';
const IMPORT_DOMAIN_FORMAT = 'api.%s.%s.arcpublishing.com';
...
functions.getExecutionContext = () => {
  ...
  const importDomain = process.env.ENV === 'prod'
    ? util.format(IMPORT_DOMAIN_FORMAT_PROD, process.env.ORG)
    : util.format(IMPORT_DOMAIN_FORMAT, process.env.ENV, process.env.ORG);
